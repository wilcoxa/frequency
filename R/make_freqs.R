makefreqs <- function(x, var, maxrow, trim){
  ...
  sort_by <- switch(getOption("freq_sort_by"),
                    "value" = var,
                    "label" = "label",
                    "count" = "freq")
  ...
  res_v <- res_v[mixedorder(res_v[[sort_by]], decreasing = getOption("freq_sort_descending")),]

  res_m <- res[res[["missing"]] %in% TRUE,]
  res_m <- res_m[mixedorder(res_m[[sort_by]], decreasing = getOption("freq_sort_descending")),]
